...
function initUI() {
  ...
  setupSelect('player', [{value:'all',label:'All players'}], value=>{state.player=value; updatePathStatus(); draw();});
  for (const key of ['showHumans','showBots','showPaths','showMarkers']) $(`#${key}`).onchange=()=>{state[key]=$(`#${key}`).checked;updatePathStatus();draw();};
...
function updatePathStatus(){
  const node=$('#pathStatus');
  if(!node)return;
  if(!state.selected){node.textContent='Select a match to view paths';return;}
  const players=state.selected.players.filter(p=>state.player==='all'||p.id===state.player);
  const humanPaths=players.filter(p=>p.human&&p.path.length>1).length;
  const botPaths=players.filter(p=>!p.human&&p.path.length>1).length;
  const humanVisible=state.showHumans?humanPaths:0;
  const botVisible=state.showBots?botPaths:0;
  node.textContent=`${humanVisible} human · ${botVisible} bot paths visible`;
  node.classList.toggle('muted',botVisible===0);
}
function refresh() {
  state.playing=false;
  ...
  setSelectValue('layer',state.layer); setSelectValue('speed',String(state.speed)); setupTimeline(selected); updatePathStatus(); draw();
  syncLayerButtons();
}
...
function drawMatch(ctx,match,w,h){const cutoff=match.start+state.time/state.timeScale;for(const p of match.players){if(state.player!=='all'&&p.id!==state.player)continue;if((p.human&&!state.showHumans)||(!p.human&&!state.showBots))continue;if(state.showPaths&&state.time===0&&p.path.length>1){ctx.beginPath();p.path.forEach((v,i)=>{const [x,y]=pointToPixel(match.map,v[1],v[2],w,h);i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.strokeStyle=p.human?'rgba(0,245,212,.2)':'rgba(0,183,255,.42)';ctx.lineWidth=p.human?2:2.5;ctx.lineCap='round';ctx.lineJoin='round';ctx.setLineDash(p.human?[]:[6,5]);ctx.stroke();ctx.setLineDash([]);}const path=interpolatedPath(p.path,cutoff);if(state.showPaths&&path.length>1){ctx.beginPath();path.forEach((v,i)=>{const [x,y]=pointToPixel(match.map,v[1],v[2],w,h);i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.strokeStyle='rgba(3,13,20,.94)';ctx.lineWidth=p.human?6:5;ctx.lineCap='round';ctx.lineJoin='round';ctx.setLineDash([]);ctx.stroke();ctx.strokeStyle=p.human?'#00f5d4':'#00b7ff';ctx.lineWidth=p.human?3.4:2.8;ctx.setLineDash(p.human?[]:[8,6]);ctx.shadowColor=p.human?'rgba(0,245,212,.52)':'rgba(0,183,255,.62)';ctx.shadowBlur=3;ctx.stroke();ctx.setLineDash([]);ctx.shadowBlur=0;}if(!state.showMarkers)continue;for(const e of p.events){if(e[0]>cutoff)continue;const [x,y]=pointToPixel(match.map,e[1],e[2],w,h);const color=COLORS[e[3]]||'#fff';ctx.beginPath();ctx.arc(x,y,e[3]==='Loot'?3:5,0,Math.PI*2);ctx.fillStyle=color;ctx.globalAlpha=.9;ctx.fill();if(e[3]!=='Loot'){ctx.strokeStyle='rgba(8,13,20,.9)';ctx.lineWidth=1.5;ctx.stroke();}ctx.globalAlpha=1;}}}
